...
function submatrixargmax(f::Function, A::AbstractMatrix, rows, cols; colmask::Function=x -> true, rowmask::Function=x -> true)
    ...
    return submatrixargmax(f, A, convertarg(rows, Base.size(A, 1)), convertarg(cols, Base.size(A, 2)); colmask=colmask, rowmask=rowmask)
end
...
function submatrixargmax(f::Function, A::AbstractMatrix, startindex::Int; colmask::Function=x -> true, rowmask::Function=x -> true)
    return submatrixargmax(f, A, startindex:Base.size(A, 1), startindex:Base.size(A, 2); colmask=colmask, rowmask=rowmask)
end

function submatrixargmax(A::AbstractMatrix, startindex::Int)
    return submatrixargmax(identity, A, startindex:Base.size(A, 1), startindex:Base.size(A, 2))
end
...
mutable struct rrLU{T}
    ...
    function rrLU(rowpermutation::Vector{Int}, colpermutation::Vector{Int}, L::Matrix{T}, U::Matrix{T}, leftorthogonal, npivot, error) where {T}
        npivot == Base.size(L, 2) || Base.error("L must have the same number of columns as the number of pivots.")
        npivot == Base.size(U, 1) || Base.error("U must have the same number of rows as the number of pivots.")
        length(rowpermutation) == Base.size(L, 1) || Base.error("rowpermutation must have length equal to the number of pivots.")
        length(colpermutation) == Base.size(U, 2) || Base.error("colpermutation must have length equal to the number of pivots.")
...
function rrLU{T}(A::AbstractMatrix{T}; leftorthogonal::Bool=true) where {T}
    rrLU{T}(Base.size(A)...; leftorthogonal=leftorthogonal)
end
...
function _optimizerrlu!(
    ...
    maxrank = min(maxrank, Base.size(A, 1), Base.size(A, 2))
    maxerror = 0.0
    ...
    if any(isnan.(lu.L))
        Base.error("lu.L contains NaNs")
    end
    if any(isnan.(lu.U))
        Base.error("lu.U contains NaNs")
    end
    if lu.leftorthogonal
    ...
    if lu.npivot >= minimum(Base.size(A))
        lu.error = 0
    end
...
function diag(lu::rrLU{T}) where {T}
    if lu.leftorthogonal
        return LinearAlgebra.diag(lu.U[1:lu.npivot, 1:lu.npivot])
    else
        return LinearAlgebra.diag(lu.L[1:lu.npivot, 1:lu.npivot])
    end
end
...
function Base.:\(A::rrLU{T}, b::AbstractMatrix{T}) where {T}
    size(A, 1) == size(A, 2) || Base.error("Matrix must be square.")
    A.npivot == size(A, 1) || Base.error("rank-deficient matrix is not supportred!")
...
Convert a rank-revealing LU decomposition `lu` into a `MatrixACA` representation.
"""
