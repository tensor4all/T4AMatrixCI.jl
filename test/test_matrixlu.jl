...
@testset "LU decomposition" begin
    @testset "Argmax finder" begin
        ...
        @test MCI.submatrixargmax(A, [3], [5]) == (3, 5)

        @test MCI.submatrixargmax(A, :, :) == Tuple(argmax(A))
        @test MCI.submatrixargmax(A, [1], :) == (1, argmax(A[1, :]))
        @test MCI.submatrixargmax(A, :, [1]) == (argmax(A[:, 1]), 1)

        @test MCI.submatrixargmax(A, 1) == Tuple(argmax(A))
        @test MCI.submatrixargmax(A, minimum(size(A))) == (minimum(size(A)), minimum(size(A)))
    end

    @testset "Argmax finder test_throws" begin
        @test_throws ArgumentError("rows must not be empty") MCI.submatrixargmax(rand(10, 10), 100)
        @test_throws ArgumentError("cols must not be empty") MCI.submatrixargmax(rand(10, 10), 3, [])

        @test_throws ArgumentError("rows ⊆ axes(A, 1) must be satified") MCI.submatrixargmax(rand(10, 10), [1, 100, 1000], [1])
        @test_throws ArgumentError("cols ⊆ axes(A, 2) must be satified") MCI.submatrixargmax(rand(10, 10), [1], [1, 100, 1000])
    end
    ...
    @testset "Argmax finder A::Matrix{ComplexF64}" begin
        ...
        @test MCI.submatrixargmax(abs2, A, [3], [5]) == (3, 5)

        @test MCI.submatrixargmax(abs2, A, :, :) == Tuple(argmax(abs2.(A)))
        @test MCI.submatrixargmax(abs2, A, [1], :) == (1, argmax(abs2.(A[1, :])))
        @test MCI.submatrixargmax(abs2, A, :, [1]) == (argmax(abs2.(A[:, 1])), 1)

        @test MCI.submatrixargmax(abs2, A, 1) == Tuple(argmax(abs2.(A)))
    end
    ...
    @testset "Implementation of rank-revealing LU" begin
        ...
        LU = MCI.rrlu(A)
        @test size(LU) == size(A)
        L = MCI.left(LU; permute=false)
        @test all(L .== UnitLowerTriangular(L))
        U = MCI.right(LU; permute=false)
        @test all(U .== UpperTriangular(U))
        @test MCI.left(LU) * MCI.right(LU) ≈ A
    end
    ...
    @testset "Implementation of approximate rank-revealing LU" begin
        ...
        LU = MCI.arrlu(Float64, (i, j) -> A[i, j], size(A), [1], [1])
        @test size(LU) == size(A)
        L = MCI.left(LU; permute=false)
        @test all(L .== UnitLowerTriangular(L))
        U = MCI.right(LU; permute=false)
    ...
    @testset "Approximation in rrLU" begin
        ...
        LU = MCI.rrlu(A, maxrank=4)
        @test size(LU) == size(A)
        @test length(MCI.rowindices(LU)) == 4
        @test length(MCI.colindices(LU)) == 4
        ...
        LU = MCI.rrlu(A, reltol=1e-2)
        @test size(LU) == size(A)
        @test length(MCI.rowindices(LU)) < size(A, 1)
        @test length(MCI.colindices(LU)) < size(A, 2)
        ...
        @test maximum(abs.(MCI.left(LU) * MCI.right(LU) .- A)) < 1e-2
    end
    ...
    @testset "lastpivoterror for full-rank matrix" begin
        ...
        LU1 = MCI.rrlu(A)
        @test MCI.pivoterrors(LU1) == [1.0, 1.0, 0.0]
        @test MCI.lastpivoterror(LU1) == 0.0
    end
    ...
    @testset "lastpivoterror for limited maxrank or tolerance" begin
        ...
        lu = MCI.rrlu(A, maxrank=2)
        @test length(MCI.pivoterrors(lu)) == 3
        @test MCI.lastpivoterror(lu) > 0

        lu2 = MCI.rrlu(A, abstol=0.5)
        @test MCI.lastpivoterror(lu2) < 0.5

        lu3 = MCI.rrlu(A, abstol=0.0)
        @test MCI.lastpivoterror(lu3) == 0.0
    end
    ...
    @testset "LU for matrices with very small absolute values" begin
        ...
        lu = MCI.rrlu(A, abstol=1e-3)
        @test MCI.npivots(lu) == 1
        @test length(MCI.pivoterrors(lu)) > 0
        @test MCI.lastpivoterror(lu) > 0
        @test size(lu) == size(A)
        @test maximum(abs.(MCI.left(lu) * MCI.right(lu) .- A)) < 1e-3
    end
    ...
    @testset "transpose" begin
        ...
        tlu = transpose(MCI.rrlu(A))

        @test MCI.left(tlu) * MCI.right(tlu) ≈ A'
    end
    ...
    @testset "solve by rrLU" begin
        ...
        A = L * U
        lua = MCI.rrlu(A)
        @test MCI.left(lua) * MCI.right(lua) ≈ A
