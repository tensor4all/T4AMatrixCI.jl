...
@testset "MatrixACA" begin
    @testset "3x3 matrix, real" begin
        ...
        aca = MCI.MatrixACA(A, (1, 1))

        @test MCI.ncols(aca) == 3
        @test MCI.nrows(aca) == 3

        @test MCI.npivots(aca) == 1
        @test aca.rowindices == [1]
        @test aca.colindices == [1]

        @test MCI.evaluate(aca, 1, 1) ≈ A[1, 1]
        @test aca[1, 1] ≈ A[1, 1]
        @test MCI.row(aca, 1) ≈ A[1, :]
        @test aca[1, :] ≈ A[1, :]
        @test MCI.col(aca, 1) ≈ A[:, 1]
        @test aca[:, 1] ≈ A[:, 1]

        MCI.addpivot!(aca, A, (2, 3))

        @test MCI.npivots(aca) == 2
        @test aca.rowindices == [1, 2]
        @test aca.colindices == [1, 3]

        @test aca[2, 3] ≈ A[2, 3]
        @test MCI.evaluate(aca, 2, 3) ≈ A[2, 3]
        @test aca[[1, 2], [1, 3]] ≈ A[[1, 2], [1, 3]]
        @test MCI.submatrix(aca, [1, 2], [1, 3]) ≈ A[[1, 2], [1, 3]]

        MCI.addpivot!(aca, A)

        @test MCI.npivots(aca) == 3
        @test aca.rowindices == [1, 2, 3]
        @test aca.colindices == [1, 3, 2]

        @test MCI.evaluate(aca) ≈ A
        @test aca[:, :] ≈ A
    end
