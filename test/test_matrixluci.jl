...
@testset "Test MatrixLUCI" begin
    @testset "Approximation in LUCI" begin
        ...
        luci = MCI.MatrixLUCI(A, maxrank=4)
        @test size(luci) == size(A)
        @test length(MCI.rowindices(luci)) == 4
        @test length(MCI.colindices(luci)) == 4

        ci = MCI.MatrixCI(
            MCI.rowindices(luci),
            MCI.colindices(luci),
            A[:, MCI.colindices(luci)],
            A[MCI.rowindices(luci), :]
        )
        @test MCI.colstimespivotinv(luci) ≈ MCI.leftmatrix(ci)
        @test MCI.pivotinvtimesrows(luci) ≈ MCI.rightmatrix(ci)
        ...
        luci = MCI.MatrixLUCI(A, reltol=1e-2)
        @test size(luci) == size(A)
        @test length(MCI.rowindices(luci)) < size(A, 1)
        @test length(MCI.colindices(luci)) < size(A, 2)
        @test maximum(abs.(MCI.left(luci) * MCI.right(luci) .- A)) < 1e-2
    end
    ...
    @testset "luci for exact low-rank matrix" begin
        ...
        A = p * q
        luci = MCI.MatrixLUCI(A)

        @test MCI.npivots(luci) == 3
        @test MCI.left(luci) * MCI.right(luci) ≈ A
        pivotmatrix = MCI.colmatrix(luci)[1:MCI.npivots(luci), :]
