...
using LinearAlgebra
import T4AMatrixCI: nrows, ncols, addpivot!, MatrixCI, evaluate
...
@testset "MatrixCI" begin
    @testset "Matrix util" begin
        ...
        @test A ≈ MCI.AtimesBinv(A, eye(3))
        @test A ≈ MCI.AinvtimesB(eye(5), A)
        ...
        @test eye(3) ≈ MCI.AtimesBinv(B, B)
        @test eye(3) ≈ MCI.AinvtimesB(B, B)
        @test eye(5) ≈ MCI.AtimesBinv(C, C)
        @test eye(5) ≈ MCI.AinvtimesB(C, C)

        @test A * inv(B) ≈ MCI.AtimesBinv(A, B)
        @test inv(C) * A ≈ MCI.AinvtimesB(C, A)
    end
    ...
    @testset "Empty constructor" begin
        ...
        for i in 1:10
            @test MCI.row(ci, i) ≈ zeros(25)
            @test ci[i, :] ≈ zeros(25)
        end
        for j in 1:25
            @test MCI.col(ci, j) ≈ zeros(10)
            @test ci[:, j] ≈ zeros(10)
        end
    end
    ...
    @testset "Full constructor" begin
        ...
        @test MCI.pivotmatrix(ci) == Apivot
        @test MCI.leftmatrix(ci) ≈ A[:, colindices] * inv(Apivot)
        @test MCI.rightmatrix(ci) ≈ inv(Apivot) * A[rowindices, :]

        @test MCI.availablerows(ci) == [1, 4, 5, 6, 7]
        @test MCI.availablecols(ci) == [2, 3]

        for i in rowindices, j in colindices
            @test MCI.evaluate(ci, i, j) ≈ A[i, j]
            ci[i, j] ≈ A[i, j]
        end

        for i in rowindices
            @test MCI.row(ci, i)[colindices] ≈ A[i, colindices]
            @test ci[i, colindices] ≈ A[i, colindices]
        end

        for j in colindices
            @test MCI.col(ci, j)[rowindices] ≈ A[rowindices, j]
            @test ci[rowindices, j] ≈ A[rowindices, j]
        end

        @test MCI.submatrix(ci, rowindices, colindices) ≈
    ...
    @testset "Finding pivots, trivial case" begin
        ...
        @test_throws ArgumentError MCI.findnewpivot(ci, A, zeros(Int64, 0), [2, 3])
        @test_throws ArgumentError MCI.findnewpivot(ci, A, [1, 2], zeros(Int64, 0))
        ...
        @test rank(ci) == 2
        addpivot!(ci, A, (MCI.availablerows(ci)[1], MCI.availablecols(ci)[1]))
    ...
    @testset "Finding pivots, rank-1 case" begin
        ...
        @test MCI.localerror(ci, A) ≈ A
        @test MCI.findnewpivot(ci, A) == ((3, 4), 48.0)
        addpivot!(ci, A)
        ...
        @test ci[:, :] ≈ A
        @test MCI.availablerows(ci) == [1, 2]
        @test MCI.availablecols(ci) == [1, 2, 3]
        ...
        @test_throws ArgumentError MCI.findnewpivot(ci, A)
    ...
    @testset "Cross interpolate smooth functions" begin
        ...
        cigauss = MCI.crossinterpolate(gauss)
        @test rank(cigauss) == 1
        ...
        cilorentz = MCI.crossinterpolate(lorentz, tolerance=1e-6, maxiter=10)
